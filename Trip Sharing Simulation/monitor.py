    ...
    def _average_total_distance(self) -> float:
        ...
        for activities in self._activities[DRIVER].values():
            ...
            for i in range(len(activities) - 1):
                curr_loc = activities[i].location
                next_loc = activities[i + 1].location
                drive_distance += \
                    location.manhattan_distance(curr_loc, next_loc)
            tot_distance += drive_distance
            count += 1
        return tot_distance / count
