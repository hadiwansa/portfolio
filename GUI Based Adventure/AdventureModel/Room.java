...
public class Room implements Serializable {
    ...
    public String getObjectString() {
        ...
        for (AdventureObject obj : objectsInRoom) {
            objectDescriptions.append(obj.getDescription()).append(", ");
        }

        // Removing the trailing comma and space
        if (objectDescriptions.length() > 2) {
            objectDescriptions.setLength(objectDescriptions.length() - 2);
        }
    ...
    public String getCommands() {
        List<String> standardDirections = Arrays.asList("DOWN", "NORTH", "IN", "WEST", "UP", "SOUTH");
        List<String> availableDirections = motionTable.getDirection().stream()
                .map(Passage::getDirection)
                .toList();

        return standardDirections.stream()
