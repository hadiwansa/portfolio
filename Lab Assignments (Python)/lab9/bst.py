...
class BinarySearchTree:
        ...
        if self.is_empty():
            ...
            if item <= self._root:
                if self._left is None:
                    self._left._root = item
                else:
                    self._left.insert(item)
            if item > self._root:
                if self._right is None:
                    self._right._root = item
                else:
    ...
    def rotate_right(self) -> None:
        ...
        root = self._root
        self._root = left_tree._root
        self._right._root = root
        self._right._right = right_tree
        self._right._left = left_tree._right
        self._left._root = left_tree._left
